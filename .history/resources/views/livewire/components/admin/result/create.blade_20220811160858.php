                                    ...
                                    @foreach ($results as $index => $result)
                                        <table class="table align-middle table-nowrap table-check">
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        <livewire:components.edit-title :model='$result' field='ca1' :key='$index'/>
                                                    </td>
                                                    <td>
                                                        <livewire:components.edit-title :model='$result' field='ca2' :key='$index'/>
                                                        {{-- <x-form.input style='width: 50px'  class="text-center" type='number' name='ca2[]' value="{{ $result->secondCA() }}"  autofocus /> --}}
                                                    </td>
                                                    <td>
                                                        <livewire:components.edit-title :model='$result' field='ca3' :key='$index'/>
                                                    ...
                                                    <td>
                                                        ...
                                                        <livewire:components.edit-title :model='$result' field='exam' :key='$index'/>
                                                    </td>
                                                    <td>
                                                        <x-form.input style='width: 50px'  class="text-center" type='hidden' name='period_id[]' value="{{ $result->period->id() }}"  autofocus />
                                                        <x-form.input style='width: 50px'  class="text-center" type='hidden' name='term_id[]' value="{{ $result->term->id() }}"  autofocus />
                                                        <x-form.input style='width: 50px'  class="text-center" type='hidden' name='grade_id[]' value="{{ $result->grade->id() }}"  autofocus />
                                                        <x-form.input style='width: 50px'  class="text-center" type='hidden' name='subject_id[]' value="{{ $result->subject->id() }}"  autofocus />
                                                        <x-form.input style='width: 50px'  class="text-center" type='hidden' name='student_id[]' value="{{  $result->student->id() }}"  autofocus />
                            ...
                            @else
                                <form action="{{ route('result.store') }}" method="POST">
                                    ...
                                    <div class="d-flex justify-content-center flex-wrap gap-2">
                                        <button type="submit" class="btn btn-primary block waves-effect waves-light pull-right">
                                            @if (count($results) > 0)
                                                Update
                                            @else
                                                Upload
