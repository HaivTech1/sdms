...
class Create extends Component
{
    ...
    {
        return Result::when($this->grade, function($query, $grade) {
            $query->whereHas('grade', function($query) use ($grade){
               $query->where('id', $grade);
            });
        
        when('period_id', $period_id)->where('term_id', $term_id)->where('grade_id', $grade_id)->where('subject_id', $subject_id)->get();
    }
