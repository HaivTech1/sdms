<div>
                            ...
                            <table class="table align-middle mb-0 table-nowrap">
                                ...
                                <tbody>
                                    @foreach ($cartItems as $item)
                                        <tr>
                                            <td>
                                                <img src="{{ asset('storage/'.$item->product->image()) }}" alt="{{ $item->product->title() }}" title="product-img"
                                                    class="avatar-md" />
                                            </td>
                                            <td>
                                                <h5 class="font-size-14 text-truncate"><a href="{{ route('user.product.show', $item->product->slug) }}"
                                                        class="text-dark">{{ $item->product->title() }}</a></h5>
                                            </td>
                                            <td>
                                                {{ trans('global.naira')}} {{ number_format($item->product->price(), 2) }}
                                            </td>
                                            <td>
                                                <div class="me-3" style="width: 120px;">
                                                    <input type="text" value="{{ $item->quantity }}" name="demo_vertical" data-item-id="{{ $item->id }}">
                                                </div>
                                            </td>
                                            <td>
                                            {{ trans('global.naira')}} {{ number_format($item->product->price * $item->quantity, 2) }}
                                            </td>
                                            <td>
                                                <button 
                        ...
                        <div class="row mt-4">
                            ...
                            <div class="col-sm-6">
                                <div class="text-sm-end mt-2 mt-sm-0">
                                    <a href="{{ route('user.product.checkout')}}" class="btn btn-success">
                    ...
                    <div class="table-responsive">
                        <table class="table mb-0">
                            <tbody>
                                <tr>
                                    <td>Grand Total :</td>
                                    <td>{{ trans('global.naira') }} {{ number_format($total, 2) }}</td>
                                </tr>
                                <tr>
                                    <td>Estimated Tax : </td>
                                    <td>{{ trans('global.naira') }} {{ number_format($tax, 2) }}</td>
                                </tr>
                                <tr>
                                    <th>Total :</th>
                                    <th>{{ trans('global.naira') }} {{ number_format($total, 2) }}</th>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    @section('scripts')
        ...
        $('input[name="demo_vertical"]').on('change', function() {
            ...
            $.ajax({
                url: '{{ route("user.product.update.cartitem", ["item" => ":item_id", "quantity" => ":quantity"]) }}'.replace(':item_id', itemId).replace(':quantity', updatedQuantity),
                type: 'GET',
                dataType: 'json',
            }).done((res) => {
        ...
        $('.removeItem').on('click', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route("user.product.delete.cartitem", ["item" => ":item_id"]) }}'.replace(':item_id', id),
                        type: 'GET',
                        dataType: 'json',
                    }).done((res) => {
