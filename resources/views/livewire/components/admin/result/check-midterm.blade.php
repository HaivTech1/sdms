<div>
                        ...
                        <div class="col-sm-12">
                            <div class="">
                                ...
                                <button class="btn btn-sm btn-primary" data-grade="" data-period="" data-term=""
                                    ...
                                    Results</button>
                                <button class="btn btn-sm btn-success" data-grade="" data-period="" data-term=""
                                    style="display:none" id="generateClassPDF"><i class="bx bxs-file-pdf"></i> Generate
                                    Class PDF
                                </button>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <form action="{{ route('result.export.excel') }}" method="POST" target="_blank">
                            @csrf
                    ...
                    <div class="row">
                        <form id="resultUpload" method="POST">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-4">
                                    <x-form.input style='width: 70px' class="text-center required midterm-input"
                                        type='number' name='' value="" step="0.01" onblur="validateInput(this)"
                                        disabled />
    ...
    <script>
        ...
        $('#fetchResultForm').on('submit', function (e) {
            ...
            $('#generateClassPDF').data('period', period);
            $('#generateClassPDF').data('term', term);
            $('#generateClassPDF').data('grade', grade);
            ...
            $.ajax({
                ...
                // Show buttons when results are loaded
                if (students.length > 0) {
                    $('#publishSelected').show();
                    $('#generateClassPDF').show();
                } else {
                    $('#publishSelected').hide();
                    $('#generateClassPDF').hide();
                }
        ...
        $('#generateClassPDF').on('click', function () {
            var button = $(this);
            toggleAble(button, true, 'Generating Class PDF...');
            ...
            if (!period_id || !term_id || !grade_id) {
                toastr.error('Please select class, session, and term first', 'Error!');
                toggleAble(button, false);
                return;
            }

            // Generate the URL for class PDF generation
            var url = "{{ route('result.generate-class-pdf.midterm', ['grade_id' => ':grade_id', 'period_id' => ':period_id', 'term_id' => ':term_id']) }}"
                .replace(':grade_id', grade_id)
                .replace(':period_id', period_id)
                .replace(':term_id', term_id);

            // Open the PDF in a new window to trigger download
            window.open(url, '_blank');
            
            // Re-enable button after a short delay
            setTimeout(function() {
                toggleAble(button, false);
                toastr.success('Class PDF generation initiated!', 'Success!');
            }, 1000);
        });
    </script>
    @endsection
